...
The content checklist is for teams to use throughout their project cycle, so that good content design is at the heart of what they deliver at all stages, rather than considered late in the process [(Download checklist)](resources/Content_Design_Logbook.xlsx){target="_blank" rel="noopener noreferrer"}.
...
Following best practice in writing about statistics if of increasing importance. As shown in the below example any sentence could be pulled out into a snippet and shown in a search engine to users who are searching for related information:
...
Any charts in your release should be appropriate for the data, reinforcing the key messages. For more detailed information on how to build charts in EES, visit our [EES charts guidance section](ees.html#Data_block_charts). 

More complex charts cannot currently be built in EES. You should only use complex charts where there is a clear user need, as simple charts are the easiest for users to understand. 

For any charts made outside of EES you must be using [GDS colours](https://design-system.service.gov.uk/styles/colour/), the following hex codes represent the colours that are available for charts in EES, we strongly recommend you stick to these (if you can't for any reason, please get in touch with us so we can understand why - we are in the process of developing more detailed guidance):

...
As statistics producers we require a variety of tools to be able to process our data. There are a wealth of tools out there to process and analyse data, and therefore it is important that we can narrow this to ones that best meet our needs, and that we can work together on, sharing knowledge across the DfE community. As a part of developing resources to help us integrate the principle of RAP into our processes, it is important that we set a clear direction on what tools we will be actively supporting teams to use. Below are the recommended tools that will give us the most power to do what we need, and will be best supported with resources for developing the necessary skills to use them. These are tools that have large user communitites in DfE, and are already working in our current IT setup.

<div class="alert alert-dismissible alert-warning">
We are happy for people to use whatever tools they want, as long as the proccesses involved meet the [principles of RAP](https://rsconnect/rsc/stats-production-guidance/rap.html#core_principles){target="_blank"}.
</div>
...
As a general approach, many people find it useful to use the resources below to get to grips with the basics of the languages first and get their head around the concepts of writing code. Essentially this is just typing out the instructions for their computer to do the processing for them. Some of the guides below for R are particularly good for walking you through this step by step.

Once you have a handle on writing code, and the kinds of things we can do with specific languages, then getting a friend or colleague to show us around the environment such as MSSMS or RStudio, and how to access real data is a critical step. It's important not to overlook the practical element of how you use these tools on your own machine with your own data, as you will need to be using these tools in your day to day processes to properly start learning. It's like driving a car, you can have as many lessons as you want, but the true experience comes from being left alone in full control of the machine!
...
- There is also a handy [general data science resources list](https://github.com/Chris-Engelhardt/data_sci_guide){target="_blank"}, that includes plenty of resources for R and SQL, with Python, Git, and shell included in there too.
...
SQL provides us with an intuitive language to query databases and manipulate the data that we have in them. Once you have downloaded MSSMS, and gained access to your particular server and database, you can start writing SQL queries to extract and manipulated your data.
...
- If you want a useful guide for R syntax or functions, then look no further than the [R cheat sheets](https://rstudio.com/resources/cheatsheets/){target="_blank"}, these can be an invaluable point of reference. Below we've included a few particularly relevant ones:
    ...
    + Visualisations with the [ggplot2 package](){target="_blank"}
...
- Adam Robinson and Zach Waller have produced [guidance for how to use git in Azure DevOps (formally VSTS)](https://dfe-analytical-services.github.io/vsts-for-analysis/){target="_blank"}, which gives a very detailed guide on how to use version control software with use in DfE analysis.
...
- Microsoft have produced this guidance on [using Git](https://docs.microsoft.com/en-us/azure/devops/user-guide/code-with-git?view=azure-devops) with [AzureDevOps](https://docs.microsoft.com/en-us/azure/devops/user-guide/what-is-azure-devops?view=azure-devops){target="_blank"}.
...
For more information on how to use these tools in your production processes, please see the [production processes](https://rsconnect/rsc/stats-production-guidance/rap.html){target="_blank"} page.

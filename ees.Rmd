...
There is no overlap between the two environments and content create on one cannot be moved to the other. We recommend that teams use the test environment, you can dummy publishing a publication by following the process for publishing. Publications will go out every hour on test, as opposed to only 9:30 on production, this means you can schedule a release for today's date and watch it start the publishing process within a hour.
...
Analysts are able to test the service within the EES test environment, and it is reccommended that production teams start here and test using the service until they are confident they know how they want to build their release for real (again, please contact the [Statistics Development Team](mailto:explore.statistics@education.gov.uk) to request access).
...
You can create static html tables for presenting information that isn't embedded in a datablock. However, these tables should only be used for data that is available in the underlying files (perhaps presented in a way that is not possible in a data block), or for textual information. All data included in your content should be available in the downloadable underlying data files.
...
Once the drafting has finished, a methodology should be set as 'appoved for publication'. After this has happened, it will then appear in the drop down menue to attach to a publication. The methodology will then be published to the public side of the platform with the next release.

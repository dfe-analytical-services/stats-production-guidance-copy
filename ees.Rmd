...
Explore Education Statistics (EES) is the Department’s new official statistics dissemination platform, designed to make DFE’s published statistics and data easier to find, access, use and understand.
...
Maintenance and BAU running of the platform is supported by the [Statistics Development Team](mailto:explore.statistics@education.gov.uk).
...
Teams can use the pre-production environment to familiarise themselves with the platform and test out what is possible. The core functionality across the environments is identical except for new changes, which are deployed through the different environments in order before they make it to production. 
...
More information on roles and permissions is available at LINK. 
...
The following roles exist within EES admin: 

* **BAU**

    Full access including administrative tools, typically only used by the Statistics Development Team and EES developers.

* **Publication owner**

    This permissions level gives publication owners control over their publications. As well as being able to manage the publication details, the publication owner also has access to:
...
* **Analyst - contributor**
    ...
    Any analyst working on a release within EES admin will have the contributor role.

* **Analyst - approver**

    The responsible statistician for a statistics release will have the approver role, allowing them to sign off releases for publication. 
    
    The release approver will usually be the responsible G5 or G6 for the statistics release. 

* **Pre-release**

    Any user invited for pre-release is given a pre-release user role. 


Within these roles we also grant specific release permissions which allow us to make sure releases are only visible by the relevant production team. 
...
If you have not published on EES before, or if you're creating a brand new publication, you may need to contact us to create the publication for you before you can get started. 
...
+ The [time period](https://rsconnect/rsc/stats-production-guidance/ud.html#list_of_allowable_time_values){target="_blank" rel="noopener noreferrer"} of the release you’re creating
...
Publication details can be managed by publication owners or via requests to the [Explore Statistics Mailbox](mailto:explore.statistics@education.gov.uk). More information at LINK. 
...
New releases can be created by publication owners or via requests to the [Explore Statistics Mailbox](mailto:explore.statistics@education.gov.uk). More information at LINK. 
...
Here you will update your tidy data files and accompanying metadata. You need to make sure that the data has passed through our [data screener checks](https://rsconnect/rsc/dfe-published-data-qa/) before trying to upload it. 
...
You'll need to give a 'subject title' to each data file you upload. The subject name should be a simple user-friendly title for the data included within the file - it is what users will see wherever the file is referenced within EES. The actual file name and metadata guidance can include more technical / coverage information.

[LINK TO FILE NAME STANDARDS FROM UD.RMD]
...
* Your subject names should be short and snappy, clearly explain what is in each file and include date ranges. Some good examples of this in practice are included below:
  + Early years provision provider type
...
[ADD CHANGING SUBJECT NAME GUIDANCE]
...
There is an option to choose if a table is a 'fea tured table' when saving each data block, here you can name the table and add a description giving the table coverage:
...
* Keep them simple
* Use only colours that are also available in EES (listed in table below)
...
[GUIDANCE FOR STYLING NON EES CHARTS IS AT CONTENT LINK]
...
In the content tab you can now start creating your release, embedding the datablocks you’ve created as you go. 
...
`r knitr::include_graphics("images/EES-release-status-history.PNG")`
...
If you have not created or published a methoodlogy page on EES before, the first thing your publication owner will need to do is create a methodology page. 

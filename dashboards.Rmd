...
We recommend using R Shiny as we feel it is the tool that best meets the service standards. It aligns with the departments strategy for RAP in Official Statistics, and the general direction of the government analytical community. It takes advantage of transferable knowledge and shared resources to minimise the costs and maximise the shared benefits.

The following are further benefits to both R and Python:

- they are open source (Service standards [12](https://www.gov.uk/service-manual/service-standard/point-12-make-new-source-code-open) and [13](https://www.gov.uk/service-manual/service-standard/point-13-use-common-standards-components-patterns))
- give more flexibility in formatting
- give more flexibility in meeting accessibility requirements
- allows us to develop automated testing and CI to QA our dashboards
- aligns with the Cabinet Office call to move towards R and Python for analysis

The guidance on this page will focus on R Shiny as this is the recommended route for Official Statistics dashboards, and has an established process to follow. If you are not using R Shiny then you should make sure you are confident in the decision you've made and can evidence that against Service Standard [11. Choose the right tools and technology](https://www.gov.uk/service-manual/service-standard/point-11-choose-the-right-tools-and-technology).
...
All dashboards published to accompany Official Statistics should have approval from the Senior Statistician repsonsible for the original release and/or the HoP for statistics at DfE. Approvals should be sent to the Statistics Development Team when asking for a new dashboard to be hosted on shinyapps.io.  

The standards for dashboards, and department strategy is maintained by the Central Statistics Standards Unit and governed by the Statistics Leadership Group, which is made up of all senior statisticians owning Official Statistics publications in the department.
...
Testing is a way to capture desired behaviour of your code, in such a way that you can automatically verify that it keeps working the way you expect. It is essential for making sure that code works the way that you intend it to, and keeps working even after you make changes to the code. You need to test your service regularly as part of quality assurance (QA) to make sure that it:
...
For dashboards created in R Shiny, we generally recommend a mix of unit tests and UI tests (integration), run using GitHub Actions for continuous integration (CI). More information and details on how to get started with this can be found in the [testing R Shiny](#testing-r-shiny) section below.
...
Collecting the right performance data means you’ll be alerted to potential problems with your service. And when you make a change to the service, you’ll be able to tell whether it had the effect you expected. In practice this will often mean setting up analytics and feedback surveys for dashboards and monitoring the data you get back.
...
For more guidance on how to peer review, see the [peer review section of the Duck Book](https://best-practice-and-impact.github.io/qa-of-code-guidance/peer_review.html).

<!-- Other things to consider 

Service monitoring - how much of that can we set up centrally, what can teams do for themselves?

Expanding on and giving templates for assessing performance
-->
...
When using R Shiny you can choose to create an app from scratch or by using a specific dashboard package such as [shinydashboard](https://rstudio.github.io/shinydashboard/). <!-- Sarah is this still right, or should we be steering people away from shinydashboard? -->

A straight shiny app gives full flexibility, but may take more effort to get your dashboard to look how you want it to. The shinydashboard package gives a simplified template for Shiny, it has less flexibility but gives quick and simple structures to help create your dashboard. <!-- Sarah is this still right, or should we be steering people away from shinydashboard? -->
...
Our template shiny app [LINK to follow soon] repository should be used a starting point for all public facing dashboards as it gives the right set up for publishing and provides template code for common parts.
...
Some initial thing around consistency and coherence, we expect that all applications published meet the following?
...
All dashboards need to meet the [latest accessibility regulations](https://www.w3.org/TR/WCAG21/).
...
**The [shinya11y](https://github.com/ewenme/shinya11y) package** is a very useful tool to look at accessibility checks across your app. Simply install and load the package, then include `use_tota11y()` at the top of your UI script. This brings up an interface that helps you examine different accessibility aspects in your app, like colour contrast, alt-text and what screen readers will detect:

`r knitr::include_graphics("images/shinya11y_demo.gif")`

**The Lighthouse feature in Google Chrome** is another easy way to quickly rate your accessibility. Open your app in Chrome, right click anywhere on the page and select "Inspect". From there, navigate to "Lighthouse" in the top grey bar, and click "Generate report". This generates scores for accessibility, best practices and SEO for your application or web page:

`r knitr::include_graphics("images/lighthouse.gif")`


Automated tools can't check for everything. Actively consider what users need from your dashboard - can they get that from the alt text etc. that you have provided? Everything that you can see, you should also be able to read with a screen reader.
...
All dashboards should have a version control page, accessibility statement, link to the source code, and information about who to contact for feedback/issues.
...
https://www.gov.uk/guidance/style-guide/a-to-z-of-gov-uk-style - should also reference this
...
To ensure that they are reliable, apps/dashboards should have an appropriate amount of automated tests before they can be published. The type of tests that you need to run will depend on the content of your application.
...
An example of a basic shiny test to check that your app loads up correctly can be found in our Shiny template repo [LINK TO TEMPLATE REPO HERE]
...
Unit testing checks, for a set of defined variables, that the output is always the same when you run your application. This ensures that if slight tweaks are made to your custom function, or functions within packages are updated, then the custom function still works.
...
- Store the data within the repo (e.g. CSV files in a /data folder)
...
* A finished app that meets the accessibility and styling standards
* Approvals from your DD, Legal, and Comms
* Code in a GitHub repo
* GitHub Actions set up

To set up a new app send these to statistics.development@education.gov.uk

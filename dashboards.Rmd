...
This guidance is aimed primarily at dashboards published to supplement Official Statistics to the wider public. See the [how to publish a dashboard](#how-to-publish-a-dashboard) section below for more information on the guidance for internal only dashboards.
...
The standards for dashboards, and department strategy is maintained by the Central Statistics Standards Unit and governed by the Statistics Leadership Group, which is made up of all senior statisticians responsible for Official Statistics publications in the department.

The statistics leadership group will review exisiting dashboards on a semi-regular basis to ensure coherence and wider awareness of the work being done.
...
At a minumum you should be requesting feedback from users via a survey hosted on the dashboard and reviewing this on a regular basis. <!-- TODO: Add in how to set up google analytics (or equivalent)-->
...
Our template shiny app [Link to follow soon] repository should be used a starting point for all public facing dashboards as it gives the right set up for publishing and provides template code for common parts. You can create your own repo from the template by using the 'use template' button available in GitHub:
...
In line with [recent legislation]() for public sector websites, all dashboards need to meet the latest [Web Content Accessibility Guidance](https://www.w3.org/TR/WCAG21/).

As a minimum we expect all dashboards to be checked using at least one of the tools below, and have an accessibility statement, before being published. 
...
Teams should pick from the [gov.uk design system colours](https://design-system.service.gov.uk/styles/colour/) when creating dashboards. The [style sheet]() in our [template repository]() makes use of these colours, though this can be extended if needed to suit your needs. Be careful when choosing colours, and make sure that colours have sufficient contrast to be placed next to each other to meet WCAG AA standards using the [colour contrast checker](https://webaim.org/resources/contrastchecker/).
...
Something on UI tests with shiny (and link to here https://dfe-analytical-services.github.io/good-code-practice/testing.html#testing-shiny-apps)
...
Something on unit tests with testthat (and link to here https://dfe-analytical-services.github.io/good-code-practice/testing.html#testthat. For more guidance, see see our guidance on testing R code section on the [learning resources page](l+d.html#R).
...
If you’ve started from our [template repository]() this will mostly be set up for you, but if not you’ll need to add the following folder to your repo [LINK](). 

You should also use GitHub Actions to run the automated tests for your app, which we recommend are done on at least every pull request. An example yaml file for running the tests is included in the [template repository](). 

GitHub actions are already well documentated and [their own guidance](https://docs.github.com/en/actions) should be able to walk you through anything you need. That being said, if there's anything else you'd like to see here let us know.

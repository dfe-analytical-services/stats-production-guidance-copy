...
<!-- this link doesn't seem right, can we point at the repo rather than specifically at prod? -->
...
<!-- links to update -->
For a collection of relevant resources to use when learning SQL, see our [learning resources](l+d.html) page, and for guidance on best practice when writing SQL queries, see the [using code to process data](#using-code-to-process-data) and [documentation](#documentation) sections on this page, as well as the guides immediately below on how to setup and use a SQL database. 
...
We should all be using code to process our data, moving away from a reliance on Excel, SPSS, and manual processing. In order to carry out our jobs to the best of our ability it is imperative that we use the [appropriate tools](#appropriate-tools) for the work that we do.
...
## Appropriate tools <!------- Hexagon / Good ------> 
...
By following [our guidance](#version-controlled-final-versions-of-code) in saving versions of code in an Azure DevOps, we will then be able to mirror those repositories in a publicly available GitHub area.
...
Clean code is the final version of code that was used to produce the files for the publication. This code should meet the best practice standards below (for SQL and R). If you are using a different language, such as Python, then contact us for advice on the best standards to use when writing code. 
...
Look at your current publications and imagine you have a new starter who understands the tools you've used but has never worked in your area before, and then review if you have documented your processes in a way that they could pick it up without assistance from your or anyone else.
...
For now, take a look at at the [resources for learning Git](l+d.html#git){target="_blank"} on the learning resources page.
...
This means having the final copies of code and documentation saved in a git-controlled Azure DevOps repo in the DISD Publilcations area.
...
The first steps are to get your final versions of code and documentation together in a single folder, and then to contact us to help you set up a repository in our DISD Azure DevOps area.
...
Until more guidance appears here, please refer to the resources above, or the other links on the [learning resources](l+d.html#git){target="_blank"} page in order to learn how to use git in practice.

...
- [How to screen your data against these standards and next steps](#how-to-check-against-the-standards)
...
Variable names must be in the first row of your file as the header row, must not include spaces, and ideally be formatted in [snake_case](https://en.wikipedia.org/wiki/Snake_case){target="_blank"} for ease of use. 
...
If you are including percentages and want to show 79%, you would have the value 79 in your data file and specify % as the unit for that column in the EES metadata. As a default EES will round decimals to 2 d.p., if you would prefer to display your data to 1 d.p. please apply this rounding to your data files before uploading to the platform.  

Likewise with financial data, don't include £ in the data file, leave those cells as numeric only, include it as an indicator unit instead. In this situation it would be good practice to include a £ symbol at the end of the variable name like `total_expenditure_£`, to make it clear to users when downloading the csv file.

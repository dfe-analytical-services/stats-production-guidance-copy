...
- 100mb to 500mb is fairly large file and may sometimes struggle to upload - [contact us](mailto:explore.statistics@education.gov.uk) if you have any issues
- 500mb and over are pretty large, will face issues uploading to EES currently (though we are working on a solution), and can also start to be difficult to handle, as you may be aware if you have one!
- Files that are over 1gb in size may currently have issues in the platform, please [contact us](mailto:explore.statistics@education.gov.uk) if you have any this large
...
We expect that any country codes used will come from [this list](https://geoportal.statistics.gov.uk/datasets/countries-december-2018-names-and-codes-in-the-united-kingdom/data), please contact us if you have data that doesn't match this.
...
| indicator_unit         | If this column is left blank then this will be a number by default, alternatively you can use either of the following units for financial or percentage measures - "£", "%", "pp". | 
...
<div class="alert alert-dismissible alert-info">
  An [interactive app](https://rsconnect/rsc/dfe-published-data-qa/){target="_blank"} has been developed to eventually replace the screener below. Feel free to test it out and give us any feedback you have.
</div> 

A data-screener has been developed in R to automate checks against the standards as a final stage of automated quality assurance before upload to EES. This can be run on any data file, though requires the EES metadata to be able to process the file. The R project can be found on [GitHub](https://github.com/dfe-analytical-services/ees-data-screener){target="_blank"}, from there you can download or clone the code to your machine and run this yourself.

<div class="alert alert-dismissible alert-danger">
  All data and EES metadata files must be run through the data-screener before uploading to EES.
</div> 

Teams can either run the files through the screening checks themselves - watch the [how to video](https://www.youtube.com/watch?v=D60UU5r_TrM&feature=youtu.be){target="_blank"} if you need help with setting this up on your machine - or by sending your files to the [Statistics Development Team](mailto:explore.statistics@education.gov.uk) to screen them for you.
...
1. Make sure the screener folder is saved to your c: drive or a desktop/downloads/documents folder, using a shared drive may cause issues.
2. Ensure that you have both R and RTools installed, and the versions are at least those shown in the video.
3. Ensure your proxy settings are correct as per [the video](https://www.youtube.com/watch?v=D60UU5r_TrM&t=160s){target="_blank"}, there's no harm in resetting them to make sure.
4. If the above fails you may need to fully uninstall and reinstall RStudio and RTools for it to work.

Failing all of the above, email explore.statistics@education.gov.uk, making sure to include what steps you have taken so far, and your error messages in the email.

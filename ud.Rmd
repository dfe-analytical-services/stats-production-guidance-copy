...
- [Next steps once your files are ready](#p8)
...
- Data files uploaded to Explore Education Statistics will be downloadable, therefore everything in your data must be publishable.
- Please remember to apply all of your usual suppression policies.
...
- In light of recent acessibility legislation, you should aim to include at least 3 years worth of data in the platform where possible.
...
- Data files must in comma separated values (.csv) format.
- Variable names must be in the first row of your file as the header row, and be in snake case – underscores_separating_each_element.
- There are no character or size limits in a csv file, but you should consider the restrictions the users will face when downloading and processing the data (excel has a cell character limit of 32,760 for example).
- If you need to use commas within a cell, then you must add a text delimiter such as quotes to your file to define each cell (this is often done automatically for you). 
...
- As long as your data meets the standards, particularly around time and geography, you can include as much as you want in a single file. 
...
People can tend to obsess over file size, however this is not everything, and balance of rows and columns can have a large impact on performance. We don't yet know the full extent of this, however we do know that some 100mb files process quickly, while others can take a while. A very rough guide based on size would be:
...
- 500mb and over are pretty large and can start to be difficult to handle (as you may be aware if you producing one!)
...
- time_period must contain either a four digit year, or a 6 digit year (covering the two years that the year spans, such as 201718). 
- time_period must be numeric. This so that the platform can calculate ranges and order periods in a logical manner.
...
| Acceptable time_identifier value | Corresponding time_period |
|----------------------------------|---------------------------|
| Academic year Q1                 | 4 digits                  |
| Academic year Q2                 | 4 digits                  |
| Academic year Q3                 | 4 digits                  |
| Academic year Q4                 | 4 digits                  |

| Acceptable time_identifier value | Corresponding time_period |
|----------------------------------|---------------------------|
| Financial year Q1                | 4 digits                  |
| Financial year Q2                | 4 digits                  |
| Financial year Q3                | 4 digits                  |
| Financial year Q4                | 4 digits                  |

| Acceptable time_identifier value | Corresponding time_period |
|----------------------------------|---------------------------|
| Tax year Q1                      | 4 digits                  |
| Tax year Q2                      | 4 digits                  |
| Tax year Q3                      | 4 digits                  |
| Tax year Q4                      | 4 digits                  |
...
| geographic_level | old_la_code | la_name    | new_la_code | school_name     | school_laestab | school_urn | level_methodology |
|------------------|-------------|------------|-------------|-----------------|----------------|------------|-------------------|
| Local authority	 | 373         | Sheffield  | E08000019   | Birkdale School | 3736005        | 107163     | Pupil residence   |
| Local authority	 | 373	       | Sheffield  | E08000019   | Birkdale School | 3736005        | 107163     | School location   |

---

### Allowable geographic levels

All rows must have country_code and country_name completed, regardless of geographic level.

: The required columns for each geographic_level in addition to country_code and country_name are included in the right column.
| geographic_level | required columns | notes |
|------------------|------------------|-------|
| National | No additional columns | |
| Regional | region_code, region_name | |
| Local authority | old_la_code, new_la_code, la_name | While it is common for teams to also provide regional columns for Local authority data, this is not mandatory when there is no Regional rows are included. That being said, we suggest that it is good practice to include the Regional aggregations where possible given the direct link between Local authorities and Regions. |
| RSC region | rcs_region_lead_name | For RSC region data, we mostly do not have complete data for a region, as in many cases, the data overlaps across RSC Regions. However we do generally define them into lead RSC regions where the majority of the data is from. |
...
**School, Provider, and Institution level data will upload as normal but not be read into the table tool by the platform, as it is not yet designed for this level of data. Please do not force these into the platform through using filters as that breaks the consitency that we are trying to set for the benefit of users. The data will still be accessible in the downloadable files for users to explore.**
...
| time_period | time_identifier | geographic_level | country_code | country_name | %_rate_absence_bands | %_rate_absence |
|-------------|-----------------|------------------|--------------|--------------|----------------------|----------------|

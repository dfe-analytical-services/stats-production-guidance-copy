...
- **[Time and geography](#time-and-geography)** - time and level identifiers that make up our observational units
...
For School and Provider, any data files that **only** consist of these levels are able to be uploaded and will be usable in the table tool. This solution is still not ideal, though it will allow you to include additional filters at school / provider level. Wherever there are multiple values per provider/school this should be marked as a filter, and where there is a single value per provider/school this should be marked as an indicator (e.g. school type now becomes an indicator as a school only has one value for type).

| geographic_level | required columns | notes |
|------------------|------------------|-------|
| School | school_name, school_urn, school_laestab | |
...
The geographic level can be anything (e.g. National or Local authority) depending on what your users would want to do. We recommend setting the level as low as possible to reduce the number of options that will appear in the later filter choice step.
...
| :           | When a geography code is **unavailable** | Data is missing, the geography is unknown so no code is available|
